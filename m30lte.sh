...
export LOCALVERSION="-Liquid°-X1.4-m30lte-gd2bgd910533f"
# sticker plox
function sticker() {
...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot$token/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="Build took $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s). | For <b>Samsung Galaxy M30 (m30lte)</b> | <b>$(${GCC}gcc --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g')</b>"
}
# Fin Error
function finerr() {
    curl -s -X POST "https://api.telegram.org/bot$token/sendMessage"
...
function compile() {
    ...
    make O=out ARCH=arm64 m30lte_defconfig
...
function zipping() {
    cd AnyKernel || exit 1
    zip -r9 ${TANGGAL}-Liquid°-X1.4-m30lte.zip *
    cd ..
}
sendinfo
compile
